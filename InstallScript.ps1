...
foreach ($app in $apps) {
    Write-Host "`nProcessing $($app.Name)..." -ForegroundColor Cyan
    ...
    if ($upgradeOutput -match "No applicable update found" -or $upgradeOutput -match "No installed package found") {
        Write-Host "$($app.Name) not upgraded. Attempting install..." -ForegroundColor Yellow
...
Write-Host "`nVerifying installed apps..." -ForegroundColor Cyan
...
if ($failedInstalls.Count -eq 0) {
    ...
    Write-Host "The following applications were NOT installed or upgraded correctly:" -ForegroundColor Red
